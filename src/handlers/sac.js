...
async function handleSAC(msg) {
  try {
    ...
    // Normaliza o número do admin para comparação segura
    const adminPhoneRaw = (process.env.ADMIN_PHONE || '556191731943').replace(/\D/g, '');
    const eORafael = remetente.includes(adminPhoneRaw);
...
`;
    ...
    const contextoFinal = eORafael 
      ? `COMANDO PRIORITÁRIO DO CRIADOR (Rafael Magalhães): ${promptUsuario}` 
